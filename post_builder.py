...
def to_markdown(
    ...
    if image_path:
        # Simple image reference; you can adapt this to your blog’s front matter later
...
def to_yaml(
    ...
    Assumes meta values are simple (str, list of str).
    """
...
def main() -> None:
    ...
    meta = load_metadata(args.json_path)

    image_path = args.image_path
    if not image_path:
        image_path = guess_image_path(args.json_path)

    include_prompt = not args.no_prompt

    fmt = args.format.lower()
    if fmt in ("md", "markdown"):
        output = to_markdown(meta, image_path, include_prompt=include_prompt)
    else:
        output = to_yaml(meta, image_path, include_prompt=include_prompt)
